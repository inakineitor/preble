...
class SlidingWindowHistogram:
    ...
    def current_allocation_cost_per_gpu(self):
        ...
        for i in range(self.num_gpus):
            ...
            costs.append(
                self.current_prefill_cost_per_gpu[i]
                + self.current_decode_lengths_per_gpu[i] * topt
            )
        return costs
    ...
    def update(
        ...
        self.prev_mis_rates[node] = 1 - (
            self.hit_tokens[node] / self.prompt_tokens[node]
        )
    ...
    def update_prefill_cost_per_node(self, node, runtime_idx):
        if (
            runtime_idx not in self.gpu_allocations.get(node)
            or node not in self.prev_mis_rates
            or node not in self.node_to_count
        ):
    ...
    def _remove_old_entries(self, current_timestamp):
        ...
        while self.timestamps and self.timestamps[0][0] < window_start:
            ...
            if self.histogram[node] <= 0:
                ...
                self.gpu_allocations[node] = (
                    set()
                )  # Reset the gpu allocation outside the time window
    ...
    def rename_node(self, old_node, new_node):
        if old_node in self.histogram:
            ...
            self.per_node_total_decode_lengths[new_node] = (
                self.per_node_total_decode_lengths.pop(old_node)
            )

            for gpu in range(self.num_gpus):
                self.per_node_prefill_cost[new_node] = self.per_node_prefill_cost[
                    old_node
                ]
    ...
    def get_prefill_cost(self, node: TreeNode):
        return (
            self.prev_mis_rates[node]
            * self.node_to_count[node]
            * prefill_time(node.num_tokens, node.context_length)
            / len(self.gpu_allocations.get(node))
        )  # potentionally divide by length of node.cached_gpus here
    ...
    def get_eviction_prefill_cost(self, node: TreeNode, gpu, is_large_node: bool):
        if not is_large_node:
            return 0
        return (
            self.prev_mis_rates.get(node, 1.0)
...
class EmpanadaScheduler:
    def __init__(
        ...
        for i in range(num_nodes):
            self.avg_topt_per_gpu[i].append(0.15)
        ...
        self.cache = LPRadixCache(
            histogram=self.histogram, num_gpus=self.num_gpus, lock=self.lock
        )
        ...
        self.overload_detector = TTFTWindowedOverloadedDetector(
            window_duration=timedelta(minutes=3)
        )
    ...
    def handle_split_node_histogram(self, split_nodes: Dict[TreeNode, TreeNode]):
        for child, parent_node in split_nodes.items():
            if self.is_large_node(parent_node) and not self.is_large_node(
                child
            ):  # new node is parent is now larger
    ...
    def handle_eviction(self, runtime_selected):
        ...
        if self.cache.allocated_size(runtime_selected) > current_max_tokens:
            num_tokens = (
                self.cache.allocated_size(runtime_selected) - current_max_tokens
            )
            self.cache.evict_with_runtime_id_without_removing(
                num_tokens,
                lambda node: self.evict_callback(node, runtime_selected),
                runtime_selected,
            )
    ...
    def virtual_evict_for_routing(self, leaf_node: TreeNode, runtime_selected: int):
        ...
        evicted_tree_nodes = self.cache.virtual_lru_eviction(
            num_to_evict, runtime_selected
        )
        eviction_cost = 0
        victim: TreeNode
        for victim in evicted_tree_nodes:
            eviction_cost += self.histogram.get_eviction_prefill_cost(
                victim, runtime_selected, self.is_large_node(victim)
            )
    ...
    def runtime_selector(
        ...
        **kwargs,
        ...
        with self.lock:
            ...
            if (
                leaf_node.num_tokens < leaf_node.context_so_far
            ):  # check that gpu allocation exists for important node
                ...
                runtime_idx = self.calculate_min_load_cost(
                    leaf_node, selected_gpus=range(self.num_gpus)
                )
            self.counter += 1
            self.update_gpu_allocation_for_parent(
                leaf_node, {runtime_idx}
            )  # Updated gpu allocations up till parent
            self.cache.update_allocated_size(
                leaf_node, runtime_idx
            )  # Update ref counters
            ...
            if self.enable_rebalancing:
                if (
                    leaf_node.depth - important_node.depth
                    < self.REBALANCING_CHAIN_LENGTH
                ):  # Ignore longer chains for Infercept optimizations
    ...
    def finish_request(
        ...
        request_id: str = None,
        input_ids=None,
        func_output: RequestFuncOutput = None,
    ):
        with self.lock:
    ...
    def handle_important_node_stealing(self, scheduled_idx):
        ...
        allocation_cost_per_gpu = (
            self.histogram.current_allocation_per_gpu_with_atleast_min_load(2)
        )
        allocations_with_indices = [
            (gpu_id, allocation_cost_per_gpu[gpu_id])
            for gpu_id in range(len(allocation_cost_per_gpu))
        ]
        # logger.info(allocations_with_indices)
        allocations_with_indices = list(
            sorted(allocations_with_indices, key=lambda x: -x[1])
        )
    ...
    def handle_important_node_stealing_recursive(self, allocation_cost_with_devices):
        ...
        smaller_device, smaller_device_allocation_cost = allocation_cost_with_devices[
            -1
        ]  # Last element is the smallest

        if (
            larger_allocation_cost
            < self.HIGH_LOAD_THRESHOLD * smaller_device_allocation_cost
        ):
            return
        ...
        for node, cost in self.histogram.histogram.items():
            if (
                larger_device in self.gpu_allocations.get(node)
                and self.is_large_node(node)
                and self.histogram.node_to_count[node] > 1
            ):
                rebalancing_cost = self.histogram.get_node_cost(
                    node, larger_device, tpot
                )
        ...
        if len(node_cost_for_gpu) == 1:
            ...
            if smaller_device not in self.gpu_allocations[
                node
            ] and self.overload_detector.is_node_overloaded(node, larger_device):
            ...
            if steal_n != 0:
                logger.info(
                    f"Steal {steal_n} nodes from {larger_device} to {smaller_device}"
                )
        allocation_cost_with_devices[0] = (larger_device, larger_allocation_cost)
        allocation_cost_with_devices[-1] = (
            smaller_device,
            smaller_device_allocation_cost,
        )
    ...
    def _print_helper(self, node: TreeNode, indent=0, depth=0):
        for key, child in node.children.items():
            ...
            print(
                f"{' ' * indent}{tokenizer.decode(child.value)[:20].strip()} Cached: {child.cached_gpus} Allocated: {allocated_gpus} Evicted: {child.evicted_gpus} {len(child.value)} Decode Lengths {list(child.decode_length)[:5]}"
            )
    ...
    def work_steal_low_loaded_prefixes(self):
        low_load_nodes = []
        allocation_cost_per_gpu = (
            self.histogram.current_allocation_per_gpu_with_atleast_min_load_or_zero_ref(
                2
            )
        )
    ...
    def update_overload_detector(
        self, input_ids, runtime_idx, func_output: RequestFuncOutput
    ):
        # Overload detector based on the current ttft
        leaf_node = self.cache.find_node(input_ids)
        important_node = self.get_important_node(leaf_node)
        self.overload_detector.add_data_point(
            datetime.now(), important_node, runtime_idx, func_output.ttft
        )

...
ours_gpu_configs_lpm = [
    ...
    GPUConfig(gpu_id=2, url=None, use_ssh=False, runtime_args=ours_server_args_lpm),
    GPUConfig(gpu_id=3, url=None, use_ssh=False, runtime_args=ours_server_args_lpm),
]
...
policies_to_test = [
    ...
    (
        ...
        CustomPolicyType.PROGRAMMING_ORACLE,
        baseline_gpu_configs,
        "oracle",
    ),
]
...
def gen_workloads_for_programming(configuration_to_test, policies_to_test):
    for configuration in configuration_to_test:
        ...
        for (
            ...
            server_configs,
            custom_policy_msg,
        ) in policies_to_test:  # assuming each policy has the exact same settings
...
exp_args = AllExperiments([loogle_experiment])

...
import logging
from benchmarks.exp_configs.model_equations_numpy import (
    ...
    def rename_node(self, old_node, new_node):
        if old_node in self.histogram:
            ...
            for i in range(self.num_gpus):
                self.per_node_per_gpu_allocation_cost[new_node][i] = (
                    self.per_node_per_gpu_allocation_cost[old_node][i]
                )
...
class TTFTWindowedOverloadedDetector:
    ...
    def purge_old_data(self, key, current_time):
        ...
        while (
            self.data[key]
            and self.data[key][0][0] < current_time - self.window_duration
        ):
...
class GlobalSchedulerWithTimePerf:
    ...
    def get_recomp_cost(self, node, gpu_id, histogram):
        ...
        if node.has_cached_gpu(gpu_id):
            ...
            return histogram.histogram.get(node, 1) + self.get_recomp_cost(
                node.parent, gpu_id, histogram
            )
    ...
    def get_recomp_cost_basic(self, node, gpu_id):
        ...
        if node.has_cached_gpu(gpu_id):
            ...
            return node.num_tokens * ref_cnt_cached + self.get_recomp_cost_basic(
                node.parent, gpu_id
            )
    ...
    def get_recomp_cost_basic_time(self, node, gpu_id):
        ...
        if node.has_cached_gpu(gpu_id):
            ...
            return prefill_time(
                node.num_tokens, node.context_length
            ) + self.get_recomp_cost_basic_time(node.parent, gpu_id)
    ...
    def runtime_selector(
        ...
        **kwargs,
        ...
        if (
            leaf_node.num_tokens < leaf_node.context_length - leaf_node.num_tokens
            ...
            if runtime_id_with_highest_hit_rate is None:
                ...
                if self.enable_eviction:
                    ...
                    costs = [
                        recom_costs[gpu_id] + histogram_mem_cost[gpu_id]
                        for gpu_id in range(self.num_gpus)
                    ]
        ...
        for node, cost in self.histogram.histogram.items():
            ...
            if (
                ...
                rebalancing_cost = self.histogram.get_node_cost(
                    node, larger_device, topts[larger_device]
                )
...
if __name__ == "__main__":
    ...
    start_time = time.perf_counter()
    for request in tqdm(requests):
        select = perf.runtime_selector(
            input_ids=request["input_ids"], sampling_params={}
        )
    end_time = time.perf_counter()
    print(
        f"Total time elapsed: {end_time - start_time} Throughput {num_requests/(end_time - start_time)}reqs/sec"
    )

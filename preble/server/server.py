...
def process_stream_output(chunk: dict, output: RequestFuncOutput, **kwargs):
    ...
    output.arrival_time = (
        chunk["meta_info"]["arrival_time"] - current_experiment_state_time
    )
    output.append_to_queue_time = (
        chunk["meta_info"]["append_to_queue_time"] - current_experiment_state_time
    )
                    ...
                    async for chunk in response.content:
                        ...
                        if chunk == "[DONE]":
                            ...
                            process_stream_output(
                                data, output, current_experiment_state_time=st
                            )
...
async def generate_request_helper(obj: GenerateReqInput):
    ...
    async def get_requests():
        async for chunk in async_send_request(
            text, input_ids, payload, runtime_id, url, rid
        ):
...
async def process_cleanup_selection():
    while True:
        ...
        request_router.finish_request(
            text=text,
            input_ids=input_ids,
            func_output=output_obj,
...
def start_server(
    ...
    runtime_urls="http://127.0.0.1:30000/generate",
    host="127.0.0.1",
    port=8000,
    ...
    global_scheduler = GlobalSchedulerWithTime(
        num_nodes=num_nodes, enable_eviction=True
    )
...
def start_server_and_load_models(
    model_name="mistralai/Mistral-7B-v0.1", devices=[0, 1], host="127.0.0.1", port=8000
):
    """
...
def main():
    fire.Fire({"run": start_server, "deploy_and_run": start_server_and_load_models})
